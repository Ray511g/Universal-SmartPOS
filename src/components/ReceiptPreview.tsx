    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-6">
      ...
      >
        ...
        <div className="bg-slate-50 p-8 text-center border-b-2 border-dashed border-slate-200">
           <div className="w-16 h-16 bg-navy-950 rounded-2xl flex items-center justify-center text-white mx-auto mb-4 font-black text-xl">
             SP
           </div>
           <h2 className="text-xl font-black uppercase tracking-tight">{businessName}</h2>
           <p className="text-xs text-slate-500 font-bold mt-1">Enterprise Terminal #001</p>
           ...
           {items.map((item, idx) => (
             <div key={idx} className="flex justify-between text-sm">
                <div className="flex-1">
                  ...
                  <p className="text-xs text-slate-500">{item.quantity} x {currency} {item.price.toLocaleString()}</p>
                </div>
        ...
        <div className="p-4 bg-navy-950 flex gap-2">
           <button className="flex-1 bg-white/10 hover:bg-white/20 text-white rounded-xl py-3 flex items-center justify-center gap-2 transition-all font-bold text-xs">
             <Share2 size={16} /> SHARE
           </button>
           <button onClick={() => window.print()} className="flex-[2] bg-brand-blue hover:bg-sapphire text-white rounded-xl py-3 flex items-center justify-center gap-2 transition-all font-black text-xs shadow-lg shadow-brand-blue/20">
